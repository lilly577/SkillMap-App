...
import { ArrowLeft, Video, Calendar, User, Briefcase, Clock } from "lucide-react";
...
const CompanyInterviews = () => {
  ...
  useEffect(() => {
    if (!isAuthenticated || user?.userType !== 'company') {
      toast({
        title: "Access Denied",
        description: "Only companies can view interviews",
        variant: "destructive",
      });
      navigate('/');
      return;
  ...
  const handleUpdateStatus = async (interviewId: string, status: string) => {
    try {
      ...
      toast({
        title: "Update Failed",
        description: (error instanceof Error ? error.message : "Failed to update interview status"),
        variant: "destructive",
      });
    }
  };
                    ...
                    <div className="space-y-4">
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="font-medium">{interview.specialistId.fullName}</p>
                            <p className="text-gray-600">{interview.specialistId.expertise}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
        ...
        {interviews.length === 0 && (
          <Card>
            <CardContent className="py-12 text-center">
              ...
              <p className="text-gray-500 mb-4">Schedule interviews with candidates to see them here</p>
